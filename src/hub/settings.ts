/**
 * /settings 页面 — 允许用户通过 app_token 验证身份后查看和修改 Cloudflare 配置
 */
...
export async function handleSettingsSave(
  ...
  const cfApiToken = formData.get("cloudflare_api_token") || "";
  const cfAccountId = formData.get("cloudflare_account_id") || "";
  ...
  if (cfApiToken) newConfig.cloudflare_api_token = cfApiToken;
  // Account ID 允许清空，所以特殊处理
  newConfig.cloudflare_account_id = cfAccountId;
...
function renderConfigForm(
  ...
  const maskedToken = maskValue(cfg.cloudflare_api_token || "");
  const maskedAccountId = maskValue(cfg.cloudflare_account_id || "");
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Cloudflare — 修改配置</title>
  <style>${COMMON_STYLE}</style>
</head>
<body>
  <div class="card">
    <h1>Cloudflare 设置</h1>
    <p class="desc">修改您的 Cloudflare 凭证配置</p>
    ...
    <form method="POST" action="/settings/save">
      ...
      <input name="cloudflare_api_token" type="password" placeholder="留空则不修改" />
      <p class="hint">在 <a href="https://dash.cloudflare.com/profile/api-tokens" target="_blank">Cloudflare Dashboard</a> 创建</p>

      <label>Account ID（可选）</label>
      <input name="cloudflare_account_id" placeholder="留空则清除" value="${escapeHtml(cfg.cloudflare_account_id || "")}" />
      <p class="hint">用于 Workers/R2 等 Account 级别操作</p>
      ...
      <div class="security-notice">
        ...
        <ul>
          ...
          <li>如需更高安全性，建议<a href="https://github.com/openilink/openilink-app-cloudflare">自行部署</a>本 App</li>
        </ul>
      </div>
